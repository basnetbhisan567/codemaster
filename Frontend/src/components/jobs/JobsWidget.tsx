...
export const JobsWidget = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      // Skip first job and take next 4
      setJobs(jobData.slice(1, 5));
      setStats({ total: statsData.totalJobs, remote: statsData.remoteJobs });
      setLoading(false);
    };
    fetchJobs();
  }, []);
              ...
              <Link to={`/jobs/${job.id}`}>
                <Card variant="interactive" className="p-4 h-full group">
                  ...
                  <div className="flex flex-wrap gap-1 mb-2">
                    ...
                    {job.remote && (
                      <Badge variant="success" size="sm" className="text-xs">Remote</Badge>
                    )}
                  </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
                    <div className="flex flex-wrap gap-1">
                      {job.tags.slice(0, 2).map((tag, i) => (
                        <span key={i} className="text-xs px-1.5 py-0.5 glass rounded">
                          {tag}
                        </span>
                      ))}
                    </div>
